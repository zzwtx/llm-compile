"""Helper wrappers that build and preprocess Qwen2-VL models for TRT-LLM."""
...
class Qwen2VLInputProcessorBase(InputProcessor):
    """Convert multimodal prompts into fused token IDs with preprocessing helpers.

    This processor wraps the Hugging Face AutoProcessor for Qwen2-VL / Qwen2.5-VL
    models and handles the extra TRT-LLM tokens needed to represent vision inputs and
    their rotary embeddings.
    """
    ...
    def _preprocess(self, text: dict[str, any], mm_data: dict[str, any],
        ...
        # The AutoProcessor performs tokenization + vision tensor conversion.
...
class Qwen2VisionModelBase:
    """Encapsulate Qwen2 vision encoder loading and batching for TRT-LLM."""

    def __init__(self, model_config: ModelConfig[PretrainedConfig],
                 model_class: type[PreTrainedModel]):
        """Instantiate the vision encoder on the correct device.

        Args:
            model_config: configuration containing the CUDA rank for this instance.
            model_class: the HF model class for the vision encoder (Qwen2-VL or Qwen2.5-VL).
        """
    ...
    def _to_device(
        ...
        """Move tensors recursively to the vision encoder device."""
    ...
    def _parse_and_batch_multimodal_data(
        ...
        """Batch image/video tensors and their grid metadata before encoding."""
    ...
    def forward(self, multimodal_params: List[MultimodalParams]):
        ...
        """Run the vision encoder over the batched multimodal tensors."""
...
class Qwen2VLModelBase(PreTrainedModel):
    """Base wrapped causal model that integrates multimodal embeddings for TRT-LLM."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Ensure the builder uses mrope scaling and loads the TRT-LLM causal LM."""
    ...
    def init_rotary_cos_sin_ori(self):
        """Prepare rotary embeddings for every position up to max_position_embeddings."""
    ...
    def load_weights(self, weights):
        """Load weights into the causal LLM and refresh rotary caches."""
    ...
    def add_rotary_cos_sin(self, multimodal_params: List[MultimodalParams]):
        """Attach cached rotary cos/sin tensors based on mrope positions."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Run a forward pass that fuses multimodal embeddings and supports inflight batching."""
...
class Qwen2VLModel(Qwen2VLModelBase):
    """AutoModel registration for the single-frame Qwen2-VL variant."""
...
class Qwen2_5_VLModel(Qwen2VLModelBase):
    """AutoModel registration for the multi-frame Qwen2.5-VL variant."""
